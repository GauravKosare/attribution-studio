...
| **Anthropic API** (`src/ai_insights.py`) | Generates the executive-summary paragraph Document 6 was written by hand — a genuinely generative task (writing prose from structured results), distinct from every other model in this project which computes a number. Opt-in, server-side API key only (`ANTHROPIC_API_KEY` env var), never a form field a visitor types a key into | Any other LLM API (OpenAI, etc.) — the module's prompt/response handling is provider-specific but the pattern (condense results -> constrained system prompt -> short summary) transfers directly |
