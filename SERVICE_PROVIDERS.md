...
## Causal validation, forecasting, uncertainty, orchestration, AI (Document 8)

| Tool | Why | Alternative |
|---|---|---|
| **statsmodels** (`src/causal/incrementality.py`, `src/forecasting.py`) | Two-proportion z-test + cluster-robust logistic regression for the geo-holdout incrementality test; damped-trend exponential smoothing for forecasting. Free, pure-Python-installable, no compiled backend headaches — a real consideration after this project's earlier heavier-dependency installs | `linearmodels` for more advanced panel/clustering methods if the causal side of this grows; `Prophet` for forecasting if you want built-in holiday effects (heavier install, needs a C++ toolchain on Windows) |
| **scipy.stats.beta** (`src/uncertainty.py`) | Exact, closed-form Bayesian credible intervals via Beta-Binomial conjugacy — genuine Bayesian inference with zero new dependencies, since scipy is already required | **PyMC** for a full hierarchical Bayesian model — deliberately not used here; heavy, sometimes fragile to install (needs a compiled backend), and the marginal rigor gain over Beta-Binomial + bootstrap didn't justify it for this project's scope. Named explicitly as the "if you want to go further" option in `src/uncertainty.py`'s own docstring |
| **Prefect** (`orchestration/attribution_flow.py`) | Free, open-source (Apache 2.0), fully self-hosted orchestration — no card, no cloud account, ever, for the path this project uses. A single ad-hoc run needs no server at all (Prefect 3's ephemeral in-process API); scheduling needs only `prefect server start` running locally | Airflow (heavier: needs a metadata DB + webserver + scheduler, overkill for one pipeline); Dagster (comparable positioning to Prefect, also free/OSS) |
| **Gemini → Groq → Anthropic fallback chain** (`src/ai_insights.py`) | Generates the executive-summary paragraph Document 6 was written by hand — a genuinely generative task (writing prose from structured results), distinct from every other model in this project which computes a number. Free-first: Gemini (free, best writing quality of the free options) tried first, Groq (free, independent infrastructure) as fallback, Anthropic (paid) last resort. All server-side API keys only, never a form field a visitor types into. AWS Bedrock was checked and ruled out — no free tier, same price as calling Anthropic directly | Any other LLM API — the module's prompt/response handling is provider-specific but the pattern (condense results -> constrained system prompt -> short summary, try next provider on failure) transfers directly to add more |
